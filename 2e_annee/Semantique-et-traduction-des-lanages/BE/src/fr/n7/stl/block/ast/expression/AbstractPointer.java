...
public abstract class AbstractPointer implements Expression {
	...
	public Type getType() {
		...
		Type pointerType =  replaceNamedType(this.pointer.getType());
		if (pointerType instanceof PointerType){
			result = ((PointerType)pointerType).getPointedType(); 
		} else {
			System.err.println("Type Check Error: PointerType expected found " + pointerType.toString() + " insted.");
		}
		return result;
	}
