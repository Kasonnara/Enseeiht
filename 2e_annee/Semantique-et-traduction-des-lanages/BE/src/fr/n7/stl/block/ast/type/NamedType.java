...
public class NamedType implements Type {
	...
	public Type merge(Type _other) {
		if (_other instanceof NamedType) {
			if (this.declaration.getName().equals(((NamedType) _other).declaration.getName())) { // TODO a rendre plus souple?
				return this;
			} else {
	...
	public Fragment getPrintCode(TAMFactory _factory) {
		return this.declaration.getType().getPrintCode(_factory);
	}

	@Override
	public boolean canPrintCode() {
		return this.declaration.getType().canPrintCode();
	}

	/**
	 * Si le type est un NamedType renvoi le type concret dont il est issu, sinon renvoi le type
	 */
	public static Type replaceNamedType(Type a_type){
		if (a_type instanceof NamedType){
			return replaceNamedType(((NamedType) a_type).getType());
		}else{
			return a_type;
		}
	}
