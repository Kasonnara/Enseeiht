...
import static Evenements_src.Evenement.countRealParametreNumber;

/**
 * Created by kasonnara on 21/05/17.
 */
public class EffectCollision implements EvenementEffect{
    private int x;
    private int y;
     ...
     */
    public EffectCollision(String[] params) {
        int c = Evenement.countRealParametreNumber(params);
        if (c < 4 || c > 4 ) throw new InvalideEvenementParametre("CAUSE D'ERREUR : Nombre de paramètre de l'effet de collision invalide ");
        try {
    ...
    public void executer(Game jeu) {
        jeu.getCurrentTerrain().editTerrainCollision(this.x,this.y,this.newTerrainIndice);
    }
    ...
    public void preExecuter(Terrain loadingTerrain) {
        loadingTerrain.editTerrainCollision(this.x,this.y,this.newTerrainIndice);
    }
}
